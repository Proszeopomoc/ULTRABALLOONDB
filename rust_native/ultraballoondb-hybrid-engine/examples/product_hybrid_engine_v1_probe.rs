...
use ultraballoondb_hybrid_engine::execute_product_hybrid_query;
...
use ultraballoondb_semantic::{
    build_native_structural_space, build_wave_scope, query_semantic_exact, query_topological,
    CreateSpaceOutcome, GraphScopeConfig, GraphSnapshotIndex, HybridHit, HybridWeights,
    ImportOutcome, NativeStructuralConfig, SemanticEvidenceHit, TopologicalHit, TrustFilter,
    UnknownTrustPolicy, VectorInput, VectorNormalization, VectorSpaceDescriptor, VectorStore,
};
...
fn main() {
    ...
    let hybrid_all_response = execute_product_hybrid_query(
        &snapshot,
        &store,
        &graph_index,
        &ids["anchor"],
    ...
    .unwrap();
    let hybrid_all_receipt = hybrid_all_response.receipt.clone();
    let hybrid_all = hybrid_all_response.hits;
    let hybrid_verified_response = execute_product_hybrid_query(
        &snapshot,
        &store,
        &graph_index,
        &ids["anchor"],
    ...
    .unwrap();
    let hybrid_verified_receipt = hybrid_verified_response.receipt.clone();
    let hybrid_verified = hybrid_verified_response.hits;
    ...
    assert!(!hybrid_all_receipt.trust_in_numeric_score);
    assert!(!hybrid_verified_receipt.trust_in_numeric_score);
    assert!(!hybrid_all_receipt.ann_used);
    assert!(!hybrid_verified_receipt.ann_used);
    assert!(hybrid_all_receipt.external_exact_backend_valid);
    assert!(hybrid_all_receipt.native_exact_backend_valid);
    assert!(hybrid_verified_receipt.external_exact_backend_valid);
    assert!(hybrid_verified_receipt.native_exact_backend_valid);
    assert_eq!(
        hybrid_all_receipt.database_snapshot_sha256,
        hybrid_all_receipt.graph_snapshot_sha256
    );
    assert_eq!(
        hybrid_verified_receipt.database_snapshot_sha256,
        hybrid_verified_receipt.graph_snapshot_sha256
    );
    ...
    let pass = graph_receipt.record_count == 7
        ...
        && !hybrid_all_receipt.trust_in_numeric_score
        && !hybrid_verified_receipt.trust_in_numeric_score
        && !hybrid_all_receipt.ann_used
        && !hybrid_verified_receipt.ann_used
        && hybrid_all_receipt.external_exact_backend_valid
        && hybrid_all_receipt.native_exact_backend_valid
        && hybrid_verified_receipt.external_exact_backend_valid
        && hybrid_verified_receipt.native_exact_backend_valid
        && hybrid_all_receipt.database_snapshot_sha256 == hybrid_all_receipt.graph_snapshot_sha256
        && hybrid_verified_receipt.database_snapshot_sha256
            == hybrid_verified_receipt.graph_snapshot_sha256;
    ...
    let report = format!(
        concat!(
            "{{\n",
            "  \"schema\": \"ultraballoondb.r4_9c2.product_hybrid_engine_probe.v1\",\n",
            "  \"pass\": {},\n",
            "  \"database_root\": {},\n",
            ...
            "  \"ann_used\": false,\n",
            "  \"exact_cpu_gpu_router_enabled\": true,\n",
            "  \"external_exact_backend_valid\": true,\n",
            "  \"native_exact_backend_valid\": true,\n",
            "  \"product_entrypoint_used\": true\n",
            "}}\n"
        ),
        pass,
    ...
    println!("PASS_R4_9C2_PRODUCT_BOUND_HYBRID_ENGINE_PROBE");
